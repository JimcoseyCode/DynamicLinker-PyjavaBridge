...
public class GRPCBridge extends PythonBridge {
    ...
    public GRPCBridge(String pythonFile) throws Exception {
        ...
        if (isServerRunning("localhost", 50051)) {
            System.out.println("[gRPC] Un serveur tourne déjà. Connexion directe...");
    ...
    /**
     * Vérifie si un serveur écoute sur le port spécifié.
     */
    ...
    private void lancerServeur() {
        try {
            String script = "src/main/java/fr/lirmm/bridge/core/impl/grpc/python/start_server.sh";
            File workingDir = new File("java").exists() ? new File("java") : new File(".");
            
            ProcessBuilder pb = new ProcessBuilder("bash", script);
            pb.directory(workingDir);
            pb.inheritIO();

            System.out.println("[gRPC] Aucun serveur détecté. Lancement automatique...");
